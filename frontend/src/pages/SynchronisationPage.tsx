...
export function SynchronisationPage() {
  ...
  const confirmerCandidate = async (candidate: CandidateGarmin) => {
    ...
    try {
      await api.confirmerCandidateGarmin(synchronisation.id, candidate.idExterne);
      setConfirmation('Activité importée. Recherche de la suivante…');
      const suivante = await api.lancerSynchronisationGarmin();
      setSynchronisation(suivante);
      if (suivante.statut === 'EN_COURS') await suivre(suivante.id);
      if (suivante.statut.startsWith('ERREUR')) setErreur(suivante.messageUtilisateur ?? 'La recherche Garmin n’a pas abouti.');
    }
    catch (e) { setErreur((e as Error).message); }
    finally { setActionEnCours(false); }
  };
