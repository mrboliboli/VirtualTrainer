...
class WorkoutCompletionServiceTest {
    ...
    void persistsTheLinkAndExposesCompletionData() {
        ...
        try {
            ...
            jdbc.execute("CREATE TABLE planned_workout(id UUID PRIMARY KEY,title VARCHAR(200),planned_date DATE,duration_minutes INTEGER,status VARCHAR(20)," +
            ...
            jdbc.update("INSERT INTO planned_workout(id,title,planned_date,duration_minutes,status) VALUES (?,?,?,?,?)", workoutId,
                    "Endurance facile", LocalDate.of(2026, 9, 10), 45, "PLANIFIEE");
            ...
            assertThat(service.latestCompletion()).get().extracting(WorkoutCompletionResponse::titre)
                    .isEqualTo("Endurance facile");
        } finally {
            database.shutdown();
        }
    }
